...
func (h *Handler) Handler(tweet *twitter.Tweet) {
	if tweet.InReplyToStatusID != 0 {
		...
		if err = h.publishImage("@"+tweet.User.ScreenName, filePath, tweet.ID); err != nil {
			log.Println(err.Error())
			return
		}
...
func (h *Handler) publishImage(name, filePath string, inReplyToStatusId int64) error {
	...
	if err = h.mediaUploader.UpdateStatusWithMedia(name, inReplyToStatusId, int64(media.MediaId)); err != nil {
		return errors.Wrap(err, "failed to finalize image upload")
	}
	return nil
}
